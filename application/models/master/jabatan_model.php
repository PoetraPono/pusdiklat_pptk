...
class Jabatan_model extends CI_Model {
	...
	function get_paged_list($limit=10, $offset=0, $order_column='', $order_type='asc', $search='', $fields='')
	{
		...
		return $this->db->get('T_REF_JABATAN',$limit,$offset);
	}
	...
	function getDetail($KatId){
		...
		$this->db->where('JABATAN_ID', $KatId);
		return $this->db->get('T_REF_JABATAN');
	}


	function add($datacreate){
		$this->db->insert('T_REF_JABATAN', $datacreate);
		return $this->db->insert_id();
	}

	function update($dataupdate, $idpengguna){
		$this->db->where('JABATAN_ID', $idpengguna);
		return $this->db->update('T_REF_JABATAN', $dataupdate);
	}

	function delete($id){
		// $this->insert_log_ins($id);
		$this->db->set('JABATAN_STATUS',0);
		$this->db->where('JABATAN_ID', $id);
		return $this->db->update('T_REF_JABATAN');
	}

	function aktif($id){
		// $this->insert_log_ins($id);
		$this->db->set('JABATAN_STATUS',1);
		$this->db->where('JABATAN_ID', $id);
		return $this->db->update('T_REF_JABATAN');
	}
