...
class Runningtext_model extends CI_Model {
  public function __construct(){
	...
	function get_paged_list($limit=10, $offset=0, $order_column='', $order_type='asc', $search='', $fields='', $status=0){
		$this->db->where('RUN_STATUS',$status);
		...
		return $this->db->get('T_POR_RUNNINGTEXT',$limit,$offset);
	}
	function count_all($search='', $fields='', $status=0){	
		...
		$this->db->from('T_POR_RUNNINGTEXT');
		return $this->db->count_all_results(); 
	}
	function add($data){
		$this->db->insert('T_POR_RUNNINGTEXT', $data);
		return $this->db->insert_id();
	}
	function upd($id, $data){
		$this->db->where('RUN_ID', $id);
		return $this->db->update('T_POR_RUNNINGTEXT', $data);
	}
	function getdetail($id){
		$this->db->where('RUN_ID', $id);
		$this->db->where('RUN_STATUS', 1);
		return $this->db->get('T_POR_RUNNINGTEXT');
	}
}