	...
	<div class="panel-body">
		...
		<div class="col-md-12">
					...
					<select data-placeholder="Pilih Kategori" class="clear-results required" tabindex="4" name="TINDAK_PIDANA_SECTOR_ID" id="">
						<?php if (count($bidang) >0){ ?>
							<?php foreach($bidang as $data) { ?>
	                    		<?php if ($data['SECTOR_NAME'] == $pidana['SECTOR_NAME']){ ?>
									<option value="<?php echo $data['SECTOR_ID']; ?>" selected><?php echo $data['SECTOR_NAME']; ?></option> 
								<?php } else { ?>
			...
			<div class="form-group">
				...
				<div class="col-sm-6">
					<input type="text" class="wajib form-control" id="" name="TINDAK_PIDANA_NAME" placeholder="Nama Sasaran Diklat" value="<?php echo $pidana['TINDAK_PIDANA_NAME']?>">
			</div>
