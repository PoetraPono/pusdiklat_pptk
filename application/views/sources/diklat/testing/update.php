			...
			<div class="form-group">
				...
				<div class="col-sm-4">
					<select id="addTypeTesting" class="select2 wajib" name="addTypeTesting" data-placeholder="-- Pilih Type Testing --">
						<option></option>
						<option value="1" <?php echo $Kuisioner['PROTEST_CONDITION']==1?'selected':'';?>>Offline</option>
						<option value="2" <?php echo $Kuisioner['PROTEST_CONDITION']==2?'selected':'';?>>Online</option>
					</select>
				</div>
			</div>
				...
				<div class="col-sm-8">
					<select id="addPacket" class="select2 wajib" name="addPacket" data-placeholder="-- Pilih Paket Soal --">
						...
						if(count($paketsoal)>0){
							echo '<option></option>';
							echo $Kuisioner['PROTEST_PACKET_NAME']!=""?'<option value="current" selected>'.$Kuisioner['PROTEST_PACKET_NAME'].' (Yang dipakai saat ini)</option>':"";
							foreach ($paketsoal as $k => $v) {
								echo '<option value="'.$v['PACKET_ID'].'" '.$selected.'>'.$v['PACKET_NAME'].'</option>';
							}
						} else {
...
<!-- global -->
	<script type="text/javascript">
		$(document).ready(function(){
			...
			if($('#addPacket').val()!=""){
				if($('#addPacket').val()=="current"){
					$('#listsoal').css({'display':''});
					$("#addPacketName").val("");
					$("#addPacketName").val($.trim($('#addPacket').select2('data').text).replace(' (Yang dipakai saat ini)',''));
					get_listsoal_current("<?php echo $Kuisioner['PROTEST_ID'];?>");
				}else{
					$('#listsoal').css({'display':''});
					$("#addPacketName").val("");
					$("#addPacketName").val($.trim($('#addPacket').select2('data').text).replace(' (Yang dipakai saat ini)',''));
					get_listsoal($('#addPacket').val());
				}
			}else{
				$('#listsoal').css({'display':'none'});				
			}
...
<!-- showdetail: paket soal -->
	<script type="text/javascript">
		...
		function get_listsoal_current(id=0){
	        // $('#input_there').html("table");
			$.ajax({
	            url : "<?php echo base_url('diklat/testing/json_listsoalpaket_current').'/'?>"+id,
	            type: "GET",
	            dataType: "JSON",
	            success: function(data){
	                ...
	                $.each(data, function(index) {
		            	...
		            	table += "<td>"+(data[index].QUESTION_SORT+1)+"</td>";
		            	table += "<td>"+data[index].QUESTION_VALUE+"</td>";
		            	table += "</tr>";
