    ...
    <div class="panel-body">
        <form class="form-horizontal need_validation" action="" role="form" method="post" enctype="multipart/form-data">
            ...
            <div class="form-group">
				...
				<?php 
				foreach ($whoismaxpost as $a => $b) {
					echo "Posttest tertinggi : <font color=blue>".$b['MEMBER_NAME'].", score = ".$b['max_post_test']."</font>";
				}
				foreach ($whoismaxpre as $c => $d) {
					echo "<br />Pretest tertinggi : <font color=blue>".$d['MEMBER_NAME'].", score = ".$d['max_pre_test']."</font>";					
				}
				echo "<br />";	
				$value = 0;
					foreach ($minpretest as $k => $v) {
						foreach ($maxposttest as $a => $b) {
							$value = ((($b['max_post_test']-$v['min_pre_test'])/$v['min_pre_test']) * 100);
							//echo $b['max_post_test']." - ".$b['MEMBER_NAME']."<br />";
							//echo $v['min_pre_test']." - ".$v['MEMBER_NAME']."<br />";	
							echo "Perkembangan tertinggi : <font color=blue>".$b['MEMBER_NAME'].", score = ".round($value, 2)."</font> % ";							
						}
					}
