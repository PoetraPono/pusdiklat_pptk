...
class Slider extends CI_Controller{
	...
	function __construct(){
		parent:: __construct();
		$this->load->model('portal/slider_model');
	...
	public function index(){
		$data = array();
		$this->template->display('portal/slider/index', $data);
	}
	

	public function detail($id=0){
		if($this->access->permission('read')){
			$data["popup"] = $this->slider_model->getDetail($id)->row_array();
			// $data["sig"] = $this->slider_model->getSignature($id)->row_array();
			$this->template->display('portal/slider/detail', $data);
		}
	}
	...
	public function listdataaktif(){
		...
		$data['iTotalRecords'][] = $this->slider_model->count_all($search,$order_field);
		$data['iTotalDisplayRecords'][] = $this->slider_model->count_all($search,$order_field);

		$aaData = array();
		$getData 	= $this->slider_model->get_paged_list($limit, $start, $order, $sort, $search, $order_field)->result_array();

		$no = (($start == 0) ? 1 : $start + 1);
		foreach ($getData as $row) {
			$aksi = ($row['POPUP_SHOW']==1) ? '<a data-status="0" data-link="portal/slider/delete/'.$row["POPUP_ID"].'" role="button" class="btn btn-xs btn-default btn-icon tip show_confirm" data-original-title="Sembunyikan" data-placement="top"><i class="icon-close"></i></a>':'<a href="'.base_url().'portal/slider/aktif/'.$row["POPUP_ID"].'" role="button" class="btn btn-xs btn-default btn-icon tip" data-original-title="Tampilkan" data-placement="top"><i class="icon-checkmark3"></i></a>';
				$aaData[] = array(
				'<center>'.$no.'</center>',
				...
				'<a href="'.base_url().'portal/slider/detail/'.$row["POPUP_ID"].'" role="button" class="btn btn-xs btn-default btn-icon tip" data-original-title="Lihat" data-placement="top"><i class="icon-file6"></i></a> 
				<a href="'.base_url().'portal/slider/update/'.$row["POPUP_ID"].'" role="button" class="btn btn-xs btn-default btn-icon tip" data-original-title="Ubah slider" data-placement="top"><i class="icon-pencil"></i></a> '.$aksi
	...
	public function listdatanonaktif(){
		...
		$start 		= (!$this->input->get('iDisplayStart'))?0:$this->input->get('iDisplayStart');
		$data['sEcho'] = $this->input->get('sEcho');
		$data['iTotalRecords'][] = $this->slider_model->count_allnonaktif($search,$order_field);
		$data['iTotalDisplayRecords'][] = $this->slider_model->count_allnonaktif($search,$order_field);


		$aaData = array();
		$getData 	= $this->slider_model->get_paged_listnonaktif($limit, $start, $order, $sort, $search, $order_field)->result_array();
		$no = (($start == 0) ? 1 : $start + 1);
		foreach ($getData as $row) {
			$aaData[] = array(
				'<center>'.$no.'</center>',
				...
				'<a href="'.base_url().'portal/slider/detail/'.$row["POPUP_ID"].'" role="button" class="btn btn-xs btn-default btn-icon tip" data-original-title="Lihat" data-placement="top"><i class="icon-file6"></i></a> '.
				'<a href="'.base_url().'portal/slider/aktif/'.$row["POPUP_ID"].'" role="button" class="btn btn-xs btn-default btn-icon tip" data-original-title="Aktifkan" data-placement="top"><i class="icon-checkmark3"></i></a>');
			$no++;
		}
		$data['aaData'] = $aaData;
		$this->output->set_content_type('application/json')->set_output(json_encode($data));
	...
	public function create(){
		if($this->access->permission('create')){	
			if($post = $this->input->post()){
				...
				$this->slider_model->add($datacreate);
				...
				if($insDb > 0){
					...
					redirect(base_url().'portal/slider/');
				}else{
					$notify = array(
							'title' 	=> 'Gagal!',
							'message'	=> $error,
				...
				$data = array();
				$this->template->display('portal/slider/create', $data);
			}
		}else{
	...
	public function update($id){
		if($this->access->permission('update')){	
			if($post = $this->input->post()){
				...
				$insDb = $this->slider_model->update($dataupdate, $id);
				...
				$data = array();
				$data['popup'] = $this->slider_model->getDetail($id)->row_array();
				$this->template->display('portal/slider/update', $data);
			}
	...
	function download($id){
        ...
        $part = $this->slider_model->getPartcicpantSert($id,$memberid)->row_array();
        $prog = $this->slider_model->getProgramSert($part['PROPAR_PROGRAM_ID'])->row_array();
        $sert = $this->slider_model->getDetailSert($part['PROPAR_PROGRAM_ID'])->row_array();
        // echo '<pre>';
        // print_r($sert); die;
...
public function delete($pggna_id = 0){
		...
		if($this->access->permission('delete')) {
			if($pggna_id==$pggna_idFilter) {
				...
				$del = $this->slider_model->delete($pggna_id, $dataupdate);
				$notify = array(
					'title' 	=> 'Berhasil!',
	...
	public function aktif($pggna_id = 0){
		...
		if($this->access->permission('update')) {
			if($pggna_id==$pggna_idFilter) {
				...
				$del = $this->slider_model->aktif($pggna_id, $dataupdate);
				$notify = array(
					'title' 	=> 'Berhasil!',
