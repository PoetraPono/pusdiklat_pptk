...
class Tes_Manual extends CI_Controller{
	...
	public function index(){
		$data = array();
		$this->template->display('tes/tes_manual/index', $data);
	}
...
//				if($insDb > 0){
					...
					redirect(base_url().'tes/tes_manual');
//				}else{
//					$notify = array(
//						'title' 	=> 'Gagal!',
...
//				}
			...
			$this->template->display('tes/tes_manual/update', $data);
		}else{
...
//				}
			...
			$data = array();
			$data['accList']  	= $this->pengguna_model->getListAcc()->result_array();
			$this->template->display('tes/tes_manual/create', $data);
		} else {
			$this->access->redirect('404');
		}
	}
	...
	public function detail($id=0){
		if($this->access->permission('read')){
			...
			$data["pengguna"] = $user;
			$this->template->display('tes/tes_manual/detail', $data);
		}
	}
