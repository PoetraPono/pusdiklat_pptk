...
class Tes_Online extends CI_Controller{
	...
	public function index(){
		$data = array();
		$this->template->display('tes/tes_online/index', $data);
	}
...
//				if($insDb > 0){
					...
					redirect(base_url().'tes/tes_online');
//				}else{
//					$notify = array(
//						'title' 	=> 'Gagal!',
...
//			$data['List']  			= $this->pengguna_model->ListAcc()->row_array();
			$this->template->display('tes/tes_online/update', $data);
		}else{
...
//				}
			...
			$data = array();
//			$data['accList']  	= $this->pengguna_model->getListAcc()->result_array();
			$this->template->display('tes/tes_online/create', $data);
		} else {
			$this->access->redirect('404');
		}
	}
	...
	public function detail($id=0){
		if($this->access->permission('read')){
			...
			$data["pengguna"] = $user;
			$this->template->display('tes/tes_online/detail', $data);
		}
	}
