...
class Materi extends CI_Controller{
	...
	public function index(){
		$data = array();
		$this->template->display('diklat/materi/index', $data);
	}
...
//				if($insDb > 0){
					...
					redirect(base_url().'diklat/materi');
//				}else{
//					$notify = array(
//						'title' 	=> 'Gagal!',
...
//			$data['List']  			= $this->pengguna_model->ListAcc()->row_array();
			$this->template->display('diklat/materi/update', $data);
		}else{
...
//			$data['accList']  	= $this->pengguna_model->getListAcc()->result_array();
			$this->template->display('diklat/materi/create', $data);
		} else {
			$this->access->redirect('404');
		}
	}
	...
	public function detail($id=0){
		if($this->access->permission('read')){
			...
			$data["pengguna"] = $user;
			$this->template->display('diklat/materi/detail', $data);
		}
	}
