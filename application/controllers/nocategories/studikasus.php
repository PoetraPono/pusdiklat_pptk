...
class Studikasus extends CI_Controller{
	...
	public function index(){
		$data = array();
		$this->template->display('nocategories/studikasus/index', $data);
	}
...
//				if($insDb > 0){
					...
					redirect(base_url().'nocategories/studikasus');
//				}else{
//					$notify = array(
//						'title' 	=> 'Gagal!',
...
//			$data['List']  			= $this->pengguna_model->ListAcc()->row_array();
			$this->template->display('nocategories/studikasus/update', $data);
		}else{
...
//				}
			...
			$data = array();
			$data['accList']  	= $this->pengguna_model->getListAcc()->result_array();
			$this->template->display('nocategories/studikasus/create', $data);
		} else {
			$this->access->redirect('404');
		}
	}
	...
	public function detail($id=0){
		if($this->access->permission('read')){
			...
			$data["pengguna"] = $user;
			$this->template->display('nocategories/studikasus/detail', $data);
		}
	}
