...
================================================== */
	...
	$.extend( $.fn.dataTable.defaults, {
		...
		language: {
			...
			infoEmpty: 'Menampilkan 0 sampai _END_ dari _TOTAL_ data',
...
$("form.need_validation").each(function(){
	...
	$(newform).validate({
		...
		errorPlacement: function(error, element) {
			...
			}else  if (element.parent().parent().attr("class") == "checker" || element.parent().parent().attr("class") == "choice" ) {
				error.appendTo( element.parent().parent().parent().parent().parent() );
			} 
			else if (element.parent().parent().attr("class") == "checkbox" || element.parent().parent().attr("class") == "radio" ) {
				error.appendTo( element.parent().parent().parent() );
		...
		submitHandler: function (form) {
			if(newform.hasClass('no_confirm')){
				form.submit();
			}else{
				$('#konfirmasipenyimpanan').modal('show');
				$('#setujukonfirmasibutton').on('click', function () {
					form.submit();
					$('#text_alert').html('Anda yakin menyimpan data ini ?');
				});	
			}

		}
	});
	$('select.select2',newform).on("change", function (ini) {
		newform.validate().element($(this));
	});
});

/*
$('select.select2ajax').on("change",function (ini) {

	//$('form.need_validation').validate().element($(ini));
});
$('select.select2_new').on("change", function (ini) {
	//$('form.need_validation').validate().element($(ini));
});
$('select.select2').on("change", function (ini) {
	alert($(ini).html());
	//$('form.need_validation').validate().element($(ini));
});
$('select.select2_new_clear').on("change", function (ini) {
	//$('form.need_validation').validate().element($(ini));
});*/
